...
CREATE    Function [dbo].[f_FormatTime] (@Number decimal(38,5),@Format as nvarchar(8))
RETURNS nvarchar(25)
AS
BEGIN
...
IF @Format = 'hh'
BEGIN
SET @AA = ROUND(convert(Decimal(38,2), @Number)/3600,2)
	SET @hh = Convert(nvarchar(25),@AA)
	RETURN @hh
END
IF @Format = 'mm'
BEGIN
...
Else
	If @hhh1 <= 9
SET @StrHH1 = '0' + CONVERT(nvarchar(14), @hhh1)
Else
SET @StrHH1 = CONVERT(nvarchar(14),@hhh1)
If @mmm1 <= 9
	SET @StrMM1 = '0' + CONVERT(nvarchar(3),@mmm1)
Else
	SET @StrMM1 = CONVERT(nvarchar(3),@mmm1)
If @sss1 <= 9
	SET @StrSS1 = '0' + CONVERT(nvarchar(3),@sss1)
Else
