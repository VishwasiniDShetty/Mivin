...
BEGIN
		...
		if not exists(select * from employeeinformation where EmployeeID=@EmployeeID)
            ...
            [operate],[setting],[maintain],[status],[isadmin],[interfaceid],[Company_default] ,[Email], [EmployeeRole])
            values(@Employeeid ,@employeeno,@Name,@designation ,@qualification,@address1,@address2,@phone,@operate,@setting ,@maintain,@status ,@isadmin  
            ,@interfaceid ,@Company_default,@Email, @Role)
            END 
            else
            BEGIN
            ...
            operate=@operate,setting=@setting,maintain=@maintain,[status]=@status,isadmin=@isadmin,
            Email=@Email,Company_default=@Company_default, [EmployeeRole]=@Role
