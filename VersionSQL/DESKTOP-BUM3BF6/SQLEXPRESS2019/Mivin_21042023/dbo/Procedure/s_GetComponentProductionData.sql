/****** Object:  Procedure [dbo].[s_GetComponentProductionData]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE              PROCEDURE s_GetComponentProductionData
...
--*******PROC NAME: s_GetComponentProductionData
--*******Author: Aditi, March 2004
--*******Modified: Removed 'Like' Madhavi, June 2004 	
--*******Modified: PKs in temp tables - Madhavi, Madhu June 25 2004
...
CREATE TABLE #ProdData (
	...
	CN float,
	TurnOver float
	CONSTRAINT ProdData_key PRIMARY KEY (workorderno, ComponentId))
...
select @strsql = 'INSERT INTO #ProdData (workorderno, ComponentID, Production, Rejection, CN, TurnOver)'
select @strsql = @strsql + ' SELECT workorderheader.workorderno, workorderheader.ComponentID, SUM(workorderproductiondetail.production) AS Production, '
...
select @strsql = @strsql + ' GROUP BY workorderheader.workorderno, workorderheader.ComponentID'
exec (@strsql)
