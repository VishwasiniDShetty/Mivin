...
where Cast(dbo.f_FormatTime((UtilisedTime + ISNULL( DownTime,0)),'hh') as float) <> 0
 ...
 Begin
	 ...
	 select #ProdData.MachineID,machineDescription,round(OEffy,2) as OEffy,round(AEffy,2) as AEffy,round(PEffy,2) as PEffy ,round(QEffy,2) as QEffy,AcceptedParts,RejCount,  
	 ReworkPerformed,Round(dbo.f_FormatTime(ISNULL(DownTime,0)+ISNULL(ManagementLoss,0),@timeformat),2) as DownTime,round(PPM,2) as PPM,
 ...
 Begin
	...
	where Cast(dbo.f_FormatTime((UtilisedTime + ISNULL( DownTime,0)),'hh') as float) <> 0
	 ...
	 select PlantID,round(OEffy,2) as OEffy,round(AEffy,2) as AEffy,round(PEffy,2) as PEffy ,round(QEffy,2) as QEffy,AcceptedParts,RejCount,  
	 ReworkPerformed,Round(dbo.f_FormatTime(ISNULL(DownTime,0)+ISNULL(ManagementLoss,0),@timeformat),2) as DownTime,round(PPM,2) as PPM ,
	 round(AvgMcHrRate,2) as AvgMcHrRate,
