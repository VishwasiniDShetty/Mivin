...
-- select @strsql =  @strsql + ' and ( machineinformation.machineid = ''' + @machineid + ''')'  
 select @strsql =  @strsql + ' and ( machineinformation.machineid in (' + @joined + '))'  --ER0453  
 ---mod 1  
 end  
--change  
...
-- select @strsql =  @strsql + ' where ( machineinformation.machineid = ''' + @machineid + ''')'  
 select @strsql =  @strsql + ' where ( machineinformation.machineid in (' + @joined + '))' --ER0453  
 ---mod 1  
 end  
--============To handle 16 downs ==========--
...
Begin  
		 ...
		 FROM #FinalData INNER JOIN downcodeinformation on #FinalData.DownID = Downcodeinformation.downid  
		 WHERE (Downtime > 0)  
		 group by downcodeinformation.Catagory,#FinalData.DownID Order By  downcodeinformation.Catagory,#FinalData.DownID 
...
----DR0379 Added Till Here
 ...
 --inner join #CatagorySort C on C.catagory=P.DownCatagory where DownTime > 0 Order by C.Slno--where DownCatagory in ('ProdTime','SETTING')   --DR0379
  Select P.DownCatagory as Catagory,P.MachineID,P.machineDescription,ROUND((P.DownTime/60),2) as Downtime  from #ProdDownTimeData P
 inner join #CatagorySort C on C.catagory=P.DownCatagory where DownTime > 0 Order by C.Slno--where DownCatagory in ('ProdTime','SETTING')   --DR0379
