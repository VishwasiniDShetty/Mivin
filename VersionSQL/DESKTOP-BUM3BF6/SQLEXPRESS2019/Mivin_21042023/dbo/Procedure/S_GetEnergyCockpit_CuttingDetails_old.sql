/****** Object:  Procedure [dbo].[S_GetEnergyCockpit_CuttingDetails_old]    Committed by VersionSQL https://www.versionsql.com ******/
...
S_GetEnergyCockpit_CuttingDetails '2017-09-11 08:00:00','2017-09-11 18:00:00','','''TC-02''','','Shift'    
...
If isnull(@Plantid,'')<>''      
begin      
 set @Plantname=' AND PlantMachine.Plantid=N'''+@Plantid+''''      
 print @Plantname      
end      
...
begin      
 --select @MachineName=' AND MachineInformation.MachineId=N'''+@Machineid+'''' -- geeta commented      
    select @MachineName=' AND MachineInformation.MachineId in ('+@Machineid+')' -- geeta added      
end      
...
Begin      
 ...
 ) SELECT MachineInformation.MachineID, MachineInformation.interfaceid,0,0,0,0,0,0,0,0,#GetShiftTime.starttime,#GetShiftTime.Endtime,#GetShiftTime.ShiftName FROM #GetShiftTime cross join MachineInformation      
   inner JOIN PlantMachine ON machineinformation.machineid = PlantMachine.MachineID  WHERE MachineInformation.interfaceid > ''0'' and  MachineInformation.devicetype=''5'' '      
...
Begin      
 ...
 SET @strsql = @strsql + 'insert into #Finaldata(#Finaldata.MachineID,#Finaldata.MachineInterface,UtilisedTime,Cutting_Time,components,PF,Cost,      
 Energy,Minenergy,Maxenergy,#Finaldata.StartTime,#Finaldata.EndTime,#Finaldata.Shift      
 ) SELECT MachineInformation.MachineID, MachineInformation.interfaceid,0,0,0,0,0,0,0,0,#Day.starttime,#Day.Endtime,''ALL'' FROM #Day cross join MachineInformation      
   inner JOIN PlantMachine ON machineinformation.machineid = PlantMachine.MachineID  WHERE MachineInformation.interfaceid > ''0'' and  MachineInformation.devicetype=''5'' '      
 If @Plantname<>'' and  @MachineName<>''      
 begin      
  SET @strsql =  @strsql + @Plantname + @MachineName      
...
begin      
 ...
 ) SELECT MachineInformation.MachineID, MachineInformation.interfaceid,0,0,0,0,0,0,0,0,''All'' FROM MachineInformation      
       inner JOIN PlantMachine ON machineinformation.machineid = PlantMachine.MachineID  WHERE MachineInformation.interfaceid > ''0'' and  MachineInformation.devicetype=''5'' '      
 If @Plantname<>'' and  @MachineName<>''      
 begin      
  SET @strsql =  @strsql + @Plantname + @MachineName      
  ...
  UPDATE #FinalData SET UtilisedTime = isnull(UtilisedTime,0) + isNull(t1.cycle,0) from(      
   ...
   ----/* If Down Records of TYPE-2*/      
    ...
    SUM(CASE      
     ...
     END) as Down      
    From AutoData WITH(NOLOCK) INNER Join --ER0383      
     (Select mc,Sttime,NdTime,#FinalData.StartTime as StartTime,#FinalData.EndTime as EndTime From AutoData WITH(NOLOCK) --ER0383      
   ...
   From(      
    Select mc,T1.StartTime,T1.EndTime,SUM((CAST(T1.OrginalCount AS Float)/ISNULL(O.SubOperations,1))) As Comp From       
      (      
         select mc,SUM(autodata.partscount)AS OrginalCount,comp,opn,#FinalData.StartTime,#FinalData.EndTime from autodata WITH(NOLOCK) --ER0383      
         inner join #FinalData on autodata.mc = #FinalData.machineinterface      
...
*/      
   ...
   ----/* If Down Records of TYPE-2*/      
    ...
    SUM(CASE      
     ...
     When autodata.sttime > T1.StartTime Then datediff(s , autodata.sttime,autodata.ndtime)      
     END) as Down      
    From AutoData WITH(NOLOCK) INNER Join      
     (Select mc,Sttime,NdTime,#FinalData.StartTime as StartTime,#FinalData.EndTime as EndTime From AutoData WITH(NOLOCK)      
      inner join #FinalData on AutoData.mc = #FinalData.machineinterface      
      Where DataType=1 And DateDiff(Second,sttime,ndtime)>CycleTime And      
      (msttime < #FinalData.StartTime)And (ndtime > #FinalData.StartTime) AND (ndtime <= #FinalData.EndTime)      
     ) as T1 ON AutoData.mc=T1.mc      
   ...
   From(   
    Select mc,T1.StartTime,T1.EndTime,SUM((CAST(T1.OrginalCount AS Float)/ISNULL(O.SubOperations,1))) As Comp From       
      (      
         select mc,SUM(autodata.partscount)AS OrginalCount,comp,opn,#FinalData.StartTime,#FinalData.EndTime from autodata WITH(NOLOCK)      
         inner join #FinalData on autodata.mc = #FinalData.machineinterface      
         where (autodata.ndtime>#FinalData.StartTime) and (autodata.ndtime<=#FinalData.EndTime) and (autodata.datatype=1)      
         Group By mc,comp,opn,#FinalData.StartTime,#FinalData.EndTime      
      ) as T1      
...
select T.MachineiD,T.StartTime,T.EndTime,round(kwh,2) as kwh from       
 ...
 inner join tcs_energyconsumption on tcs_energyconsumption.gtime=T.mingtime   AND tcs_energyconsumption.MachineID = T.MachineID --DR0359      
...
select T.MachineiD,T.StartTime,T.EndTime,round(kwh,2)as kwh from       
 ...
 where tcs_energyconsumption.kwh>0 --ER0419 Added      
 group by  tcs_energyconsumption.MachineiD,StartTime,EndTime)T      
 inner join tcs_energyconsumption on tcs_energyconsumption.gtime=T.maxgtime   AND tcs_energyconsumption.MachineID = T.MachineID -- DR0359      
...
begin      
 ...
 set @sql=@sql+' insert into #temp (Machine,starttime,Endtime,Cuttingdetail,TotalCuttingTime)      
  Select Machineinformation.Machineid,sttime as StartTime,ndtime as EndTime,0,0 from autodata A WITH(NOLOCK) --ER0383      
 ...
 from tcs_energyconsumption s1 WITH(NOLOCK) inner join #temp on s1.machineid=#temp.machine --ER0383      
...
begin      
 insert into #temp (Machine,starttime,Endtime,Cuttingdetail,TotalCuttingTime)      
 Select T.Machineid,sttime as StartTime,ndtime as EndTime,0,0 from autodata A WITH(NOLOCK) --ER0383      
 inner join Machineinformation mi on A.mc = mi.interfaceid      
...
select A.mc,A.Starttime,CO.Componentid + ' <' + cast(CO.Operationno as nvarchar(50)) + '>'  as CO INTO #CO from     
 (    
     Select Distinct A.mc,A.comp,A.opn,F.starttime from Autodata A WITH(NOLOCK)     
...
--ER0419 Added From Here      
 select MachineID,shift,StartTime,EndTime,CompOpn,dbo.f_FormatTime(UtilisedTime,'HH:MM:SS') as UtilisedTime,     --ER0454 added compopn 
 dbo.f_FormatTime(Cutting_Time,'HH:MM:SS') as Cutting_Time,components,Round(PF,2) as PF,Round(Cost,2) as Cost,      
