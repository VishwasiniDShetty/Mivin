/****** Object:  Procedure [dbo].[s_GetOperatorProductionData_v01]    Committed by VersionSQL https://www.versionsql.com ******/

create   PROCEDURE s_GetOperatorProductionData_v01
...
select @strsql = 'INSERT INTO #ProductionData (OperatorID, Production, Rejection, CN, TurnOver)'
select @strsql = @strsql + ' SELECT  workorderproductiondetail.employeeid, ISNULL(SUM(workorderproductiondetail.production),0) AS Production, '
...
select @strsql = @strsql + ' GROUP BY workorderproductiondetail.employeeID, workorderheader.workorderno '
exec (@strsql)
--print @strsql
/*
