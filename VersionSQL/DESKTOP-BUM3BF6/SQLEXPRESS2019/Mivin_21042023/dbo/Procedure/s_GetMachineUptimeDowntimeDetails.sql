...
Begin
		...
		SET @strSql = ''
		select @strsql = @strsql + 'Insert into #Machineupdowntime(Plantid,MachineID,Starttime,Endtime,[Value],Color,shiftdate,
			shiftname,shiftstart,shiftend,dcode)
			select P.Plantid,M.Machineid,case when A.msttime< SD.shiftstart then SD.shiftstart else A.msttime end as Starttime,
			case when A.ndtime>SD.shiftend then SD.shiftend else A.ndtime end as endtime,
			''2'',''Yellow'',
			SD.shiftdate,SD.shiftname,SD.shiftstart,SD.shiftend,A.dcode
			from #T_autodata A 
			inner join Machineinformation M on M.interfaceid=A.mc
			inner join plantmachine P on P.machineid=M.machineid 
...
Begin
			...
			case when autodata.ndtime>T.shiftend then T.shiftend else autodata.ndtime end,'3','Maroon' From #T_autodata AutoData  INNER Join
...
Begin
			...
			BEGIN
				...
				Insert into #PLD(shiftdate,shiftname,shiftstart,shiftend,Pstarttime,Pendtime,MachineID,starttime,endtime,[value],color)
				...
				CASE When autodata.ndtime>T.Pendtime Then T.Pendtime Else autodata.ndtime End As endtime,'0.5','Blue'
				From #T_autodata AutoData  CROSS jOIN 
				(select shiftdate,shiftname,shiftstart,shiftend,Pstarttime,Pendtime,MachineInterface,MachineID from #PlannedDownTimes) T
				...
				CASE When autodata.ndtime>T.EndTime Then T.EndTime Else autodata.ndtime End As Pendtime,'0.5','Blue'
				From #T_autodata AutoData  CROSS jOIN (select shiftdate,shiftname,shiftstart,shiftend,PStartTime as starttime,
			    PEndTime as endtime,MachineInterface,machineid from #PlannedDownTimes) T  INNER Join 
				...
				CASE When autodata.ndtime>T.endtime Then T.endtime Else autodata.ndtime End As Pendtime,'0.5','Blue'
				From #T_autodata AutoData  CROSS jOIN (select shiftdate,shiftname,shiftstart,shiftend,PStartTime as starttime,
			    PEndTime as endtime,MachineInterface,machineid from #PlannedDownTimes) T INNER Join --ER0324 Added
					(Select mc,Sttime,NdTime,shiftstart as StartTime,shiftend as EndTime From #T_autodata AutoData  
					 inner join #MShift P on P.Minterfaceid=autodata.mc
						Where DataType=1 And DateDiff(Second,sttime,ndtime)>CycleTime And
						(msttime < shiftstart)And (ndtime > shiftend)) as T1
			...
			BEGIN
					...
					CASE When autodata.ndtime>T.PEndTime Then T.PEndTime Else autodata.ndtime End As Pendtime,'0.5','Blue'
					...
					Insert into #MLPLD(shiftdate,shiftname,shiftstart,shiftend,Pstarttime,Pendtime,MachineID,starttime,endtime,threshold,PPDT)
					select T1.shiftdate,T1.shiftname,T1.StartShift,T1.Endshift,T2.Pstarttime,T2.Pendtime,
					T1.machineid,T1.sttime,T1.ndtime,T1.Threshold,T2.PPDT
					from
					...
					END ) as PPDT,Pstarttime,PEndtime
					From #T_autodata AutoData 
					...
					AND (downcodeinformation.availeffy = 1) group by autodata.id,T.Shiftstart,Pstarttime,PEndtime ) as T2 on T1.id=T2.id  and T1.StartShift=T2.intime 
					...
					else Dateadd(second,isnull(PPDT,0),starttime) End ,'0.5','Blue'
					FROM #MLPLD
			END
