...
--[dbo].[s_ViewComponentMaster] '','','ViewComponentInfo'
...
Begin
	...
	If @Componentid <> ''
	BEGIN
	SELECT  CI.componentid, CI.description, CI.customerid, CI.basicvalue, CI.InterfaceID, CI.InputWeight, CI.ForegingWeight,CI.IsPumpEnabled
...
Begin
	...
	BEGIN
		...
		COP.StdDieCloseTime, COP.StdPouringTime, COP.StdSolidificationTime, COP.StdDieOpenTime,COP.FinishedOperation,COP.MinLoadUnloadThreshold
...
BEGIN
	...
	COP.DrawingNo,COP.SubOperations,COP.StdSetupTime,COP.MachiningTimeThreshold,COP.TargetPercent,COP.LoadUnload as LoadUnloadTimeThreshold,C.CustomerID,COP.SCIThreshold ,COP.DCLThreshold,C.IsPumpEnabled  from 
	...
	COP.DrawingNo,COP.SubOperations,(COP.StdSetupTime/60) as StdSetupTime,COP.MachiningTimeThreshold,COP.TargetPercent,(COP.LoadUnload/60) as LoadUnloadTimeThreshold,C.CustomerID,COP.SCIThreshold ,COP.DCLThreshold,C.IsPumpEnabled  from 
	ComponentInformation C left  join ComponentOperationPricing COP on C.ComponentID= COP.ComponentID where  Cop.ComponentID in (select componentID from #component)
	END
