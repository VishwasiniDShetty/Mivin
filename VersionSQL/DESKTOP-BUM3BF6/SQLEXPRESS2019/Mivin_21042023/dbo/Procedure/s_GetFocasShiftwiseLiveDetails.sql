...
--[dbo].[s_GetFocasShiftwiseLiveDetailstest] '2017-01-03','2017-01-03','','','','hour',''                  
*************************************************************************************************************/                 
...
BEGIN            
 ...
 select @strsql =''                  
 select @strsql = @strsql + 'Insert into #LiveDetails (Machineid,ShiftDate,[From time],[To Time],ShiftName,[Cutting time],Powerontime,[Operating Time],[PartsCount],[ProgramNo])  --SV                
 SELECT distinct Machineinformation.machineid,s.[From Time],s.[From Time],s.[To time],0,0,0,0,0,0 FROM Machineinformation  --SV                
 --inner join Plantmachine on Machineinformation.machineid=Plantmachine.machineid                 
 Left outer join Plantmachine on Machineinformation.machineid=Plantmachine.machineid                   
...
Begin          
 ...
 Insert into #Totalgetparts(id,Machineid,ShiftDate,Shift,ProgramNo,Partscount,total,Partnumber,Target,OEE)                
 Select row_number() over(order by Machineid) as id, Machineid,date,Shift,ProgramID,isnull(Sum(Partcount),0)as Partscount,0 as total,ProgramBlock,0,0 from FocasWeb_HourwiseCycles                
...
Begin          
 --To Sum up all the Parts at ProgramNo Level for the given Machine and time Period.                
 Insert into #Totalgetparts(id,Machineid,ShiftDate,ProgramNo,Partscount,total,Partnumber,Target,OEE)                
 Select row_number() over(order by Machineid) as id, Machineid,date,ProgramID,isnull(Sum(Partcount),0)as Partscount,0 as tota,ProgramBlock,0,0 from FocasWeb_HourwiseCycles          
  where (partcount>0 and Convert(nvarchar(10),Date,120)>=Convert(nvarchar(10),@DataStart,120) and Convert(nvarchar(10),Date,120)<=Convert(nvarchar(10),@DataEnd,120))          
...
Begin          
 --To Sum up all the Parts at ProgramNo Level for the given Machine and time Period.                
 Insert into #Totalgetparts(id,Machineid,ShiftDate,Shift,Fromtime,Totime,ProgramNo,Partscount,total,Partnumber,Target,OEE)                
 Select row_number() over(order by Machineid) as id, Machineid,date,Shift,Hourstart,HourEnd,ProgramID,isnull(Sum(Partcount),0)as Partscount,0 as total,ProgramBlock,0,0 from FocasWeb_HourwiseCycles                
 where (partcount>0 and Convert(nvarchar(10),Date,120)>=Convert(nvarchar(10),@DataStart,120) and Convert(nvarchar(10),Date,120)<=Convert(nvarchar(10),@DataEnd,120))          
...
Begin                 
 ...
 Round(L.[Operating time],2) as [Operating time],isnull(P.ProgramNo,0) as ProgramNo,isnull(P.PartsCount,0) as PartsCount,Partnumber,P.OEE as OEE from #LiveDetails L left outer join #Totalgetparts P                
...
Begin                  
 ...
 isnull(P.ProgramNo,0) as ProgramNo,isnull(P.PartsCount,0) as PartsCount,Partnumber,P.OEE as OEE  from #LiveDetails L left outer join #Totalgetparts P                
...
/****** Object:  StoredProcedure [dbo].[s_GetFocasHourShiftwiseLiveDetails]    Script Date: 04-Dec-2018 1:32:20 PM ******/
SET ANSI_NULLS ON
