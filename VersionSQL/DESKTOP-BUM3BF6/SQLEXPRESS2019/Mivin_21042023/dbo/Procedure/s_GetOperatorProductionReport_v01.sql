/****** Object:  Procedure [dbo].[s_GetOperatorProductionReport_v01]    Committed by VersionSQL https://www.versionsql.com ******/

create    PROCEDURE s_GetOperatorProductionReport_v01
	@StartTime DateTime,
	@EndTime DateTime,
...
CREATE TABLE #Time ( tTime float, OperatorID nvarchar(50))
...
INSERT #Time (tTime, OperatorID ) EXEC s_GetOperatorProductionTime @StartTime, @EndTime, @MachineID, @ComponentID, @OperatorID
UPDATE #productiondata SET #productiondata.productiontime = isnull(#time.ttime,0)
FROM #productiondata INNER JOIN #time ON #productiondata.operatorid = #time.operatorid
--UPDATE #ProductionData SET ProductionTime = (SELECT tTime FROM #Time WHERE #Time.OperatorID = #ProductionData.OperatorID)
DROP TABLE #Time
-- Load Down Time Data
CREATE TABLE #DTime ( tTime float, AvailabilityEfficiencyLoss float, OperatorID nvarchar(50))
...
--UPDATE #ProductionData SET DownTime = isNull((SELECT tTime FROM #DTime WHERE #DTime.OperatorID = #ProductionData.OperatorID),0)
--UPDATE #ProductionData SET AvailabilityEfficiencyLoss = isNull((SELECT AvailabilityEfficiencyLoss FROM #DTime WHERE #DTime.OperatorID = #ProductionData.OperatorID),0)
DROP TABLE #DTime
