...
---mod 1 introduced with mod2 to neglect threshold ML from dtime  
 ...
 select ComponentID,round(OEffy,2) as OEffy,round(AEffy,2) as AEffy,round(PEffy,2) as PEffy ,round(QEffy,2) as QEffy,AcceptedParts,RejCount,  
 ReworkPerformed,Round(dbo.f_FormatTime(DownTime,@timeformat),2) as DownTime,round(PPM,2) as PPM
