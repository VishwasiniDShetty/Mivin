...
-- [s_GetMando_Reports] '2019-03-20 07:00:00','2019-03-21 07:00:00','','','','1','2'
*/
...
select @Strsql='Insert #MachineID(ComponentID,Component,OperationID,Operation,OperatorID,Operator,Machine ,MachineID )
		...
		---mod 2
		select @Strsql= @Strsql +'inner join employeeinformation  on employeeinformation.interfaceid=autodata.opr left outer join PlantMachine P on
		machineinformation.machineid = P.MachineID
		where sttime >= ''' +convert(varchar(20),@StartTime,120)+ ''' and ndtime <= ''' + convert(varchar(20),@EndTime,120)+ ''''
...
--select 's_GetEfficiencyFromAutodata ''' + convert(nvarchar(20),@StartDate) + ''',''' +convert(nvarchar(20), @EndDate) + ''',''' +convert(nvarchar(50), @Machine) + ''',''' + convert(nvarchar(50),@Plant) + ''','',''shift'',''' + convert(nvarchar(50),@ShiftID) + ''',''console'''
	...
	Begin
		...
		BEGIN
			...
			FROM PlannedDownTimes P cross join #GetShiftTime S
			...
			Select @strsql= @strsql+ 'from(
						select T.Shiftst  as intime,M.Machineid as machine,Downcodeinformation.DownID as inDwn,SUM
						       (CASE
					...
					Select @strsql = @strsql  +' group by M.Machineid,T.ShiftSt,Downcodeinformation.DownID ) as t2 inner join 
...
--		select @strXmachine = ' and ( EX.MachineID = ''' + @Machine + ''')'
	...
	Update #FINALOUTPUT set RowValue = t1.Tcount from (select CAST(CEILING(CAST(sum(autodata.partscount)AS Float)/ISNULL(componentoperationpricing.SubOperations,1)) AS INTEGER) as Tcount,
	...
	BEGIN
		...
		from
		( select T.Shiftst as hrst ,#FINALOUTPUT.MachineID as mach,CAST(CEILING(CAST(sum(autodata.partscount)AS Float)/ISNULL(O.SubOperations,1)) AS INTEGER) as compcnt ,
	...
	#FINALOUTPUT.Operation as opnID, #FINALOUTPUT.StartTime as strt,#FINALOUTPUT.EndTime as endt ,(RowValue-(RowValue*(Ti.Ratio)))AS Xcount
	...
	(
		...
		FROM #Exceptions
			 Inner Join ( select 
				...
				#FINALOUTPUT.Operation as opnID, #FINALOUTPUT.StartTime as strt,#FINALOUTPUT.EndTime as endt, SUM(convert(int,RowValue)) AS tCount
	...
	(select machine,component,operation,operator,sum(convert(int,rowvalue)) as TotOutput from #FinalOutput group by machine,component,operation,operator)
	...
	(select mc,sum(case
	when sttime<@StartDate and sttime<@EndDate and ndtime>@StartDate and ndtime<@EndDate  then datediff(second,@StartDate,ndtime)
	when sttime>=@StartDate and sttime<@EndDate and ndtime>@StartDate and ndtime<=@EndDate  then datediff(second,sttime,ndtime)
	when sttime>@StartDate and sttime<@EndDate and ndtime>@StartDate and ndtime>@EndDate  then datediff(second,sttime,@EndDate)
	when sttime<@StartDate and sttime<@EndDate and ndtime>@StartDate and ndtime>@EndDate  then datediff(second,@StartDate,@EndDate)
	else 0 end)as DownTime  from #T_autodata autodata where (datatype = 2 ) and (sttime<@EndDate and ndtime>@StartDate)
