...
BEGIN
		If @downid <> ''
			BEGIN
				...
				else CONVERT(bit, 0)  end as result from downcodeinformation where downid like @downid + '%'
				order by downid
			END
		If @downid = ''
			BEGIN
				...
				when 'DCL' then CONVERT(bit, 1)
				when 'DCLU' then CONVERT(bit, 1)
				else CONVERT(bit, 0)  end as result from downcodeinformation 
				order by downid
			END
END
...
BEGIN
	if @interfaceid <>''
		BEGIN
				...
				when 'DCI' then 1
				when 'DCLU' then 1
				else 0  end as result from downcodeinformation  where interfaceid like @interfaceid + '%'
	...
	if @interfaceid = ''
		BEGIN
				...
				when 'DCI' then 1
				when 'DCLU' then 1
				else 0  end as result from downcodeinformation 
			order by interfaceid
		END
END
...
BEGIN
	if @category <>''
		BEGIN
				...
				when 'DCI' then 1
				when 'DCLU' then 1
				else 0  end as result from downcodeinformation  where catagory like @category + '%'
			order by catagory
		END
