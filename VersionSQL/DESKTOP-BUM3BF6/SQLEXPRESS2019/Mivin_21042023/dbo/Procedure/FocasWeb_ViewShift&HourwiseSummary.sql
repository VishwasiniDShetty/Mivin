...
Cross Join FocasWeb_RowHeader C  where M.TPMTrakenabled=1 and Convert(Nvarchar(10),F.Date,120)=Convert(Nvarchar(10),@Date,120) order By C.Sortorder,F.ShiftID
...
--BEGIN
	...
	Inner join PlantMachine P on P.PlantID=PIN.Plantid
	Inner Join Machineinformation M on P.Machineid=M.Machineid and F.Machineid=M.Machineid
	where M.TPMTrakenabled=1 and Convert(Nvarchar(10),Date,120)=Convert(Nvarchar(10),@Date,120) order By PIN.PlantID,F.MachineID,ShiftID,HourID
--END
...
--BEGIN
	...
	where M.TPMTrakenabled=1 and Convert(Nvarchar(10),Date,120)=Convert(Nvarchar(10),@Date,120) order By PIN.PlantID, F.MachineID, F.ShiftID , HourID
...
--BEGIN
	...
	Inner join PlantMachine P on P.PlantID=PIN.Plantid
	Inner Join Machineinformation M on P.Machineid=M.Machineid and F.Machineid=M.Machineid
	where M.TPMTrakenabled=1 and Convert(Nvarchar(10),Date,120)=Convert(Nvarchar(10),@Date,120) order By PIN.PlantCode, F.MachineID,ShiftID,Batchstart
...
--END
	...
	where M.TPMTrakenabled=1 and F.Alarmtime between S.Shiftstart and S.Shiftend
