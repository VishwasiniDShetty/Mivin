/****** Object:  Procedure [dbo].[s_GetMachineProductionData]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE  PROCEDURE s_GetMachineProductionData
...
CREATE TABLE #ProdData (
	...
	CN float,
	TurnOver float
	CONSTRAINT ProdData_key PRIMARY KEY (workorderno, machineid))
...
select @strsql = 'INSERT INTO #ProdData (workorderno, MachineID, Production, Rejection, CN, TurnOver)'
select @strsql = @strsql + ' SELECT  workorderheader.workorderno, workorderheader.MachineID, SUM(workorderproductiondetail.production) AS
Production, '
...
select @strsql = @strsql + ' GROUP BY workorderheader.MachineID, workorderheader.workorderno '
exec (@strsql)
/*
INSERT INTO #ProdData (MachineID, Production, Rejection, CN, TurnOver)
...
GROUP BY workorderheader.MachineID, workorderheader.workorderno
*/
...
--	WHERE 	
--		( workorderheader.machineid LIKE '%'+#ProdData.MachineID+'%')
--		AND
--		( workorderheader.componentid LIKE '%'+@ComponentID+'%' )
--		AND
...
--	GROUP BY workorderheader.MachineID, workorderheader.workorderno
--	),0)
-- Return the totals for each machine
SELECT
	MachineID,
