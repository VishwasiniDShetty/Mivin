...
BEGIN
		...
		BEGIN
			Insert InTo ComponentInformation(Componentid, description, customerid, basicvalue, InterfaceID, InputWeight, ForegingWeight,IsPumpEnabled)
			SELECT @Componentid, @description, @customerid, @basicvalue, @CompInterfaceID, @InputWeight, @ForegingWeight,@IsPumpEnabled
			Return
		END

		IF EXISTS(select * from ComponentInformation where Componentid=@Componentid)
		BEGIN
			Update ComponentInformation SET description=@description, customerid=@customerid, basicvalue=@basicvalue, InterfaceID=@CompInterfaceID, InputWeight=@InputWeight, ForegingWeight=@ForegingWeight,IsPumpEnabled=@IsPumpEnabled
