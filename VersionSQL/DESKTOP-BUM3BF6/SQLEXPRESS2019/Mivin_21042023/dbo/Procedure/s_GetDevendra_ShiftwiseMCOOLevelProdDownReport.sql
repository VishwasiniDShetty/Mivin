...
--[dbo].[s_GetDevendra_ShiftwiseMCOOLevelProdDownReport]   '2017-05-24 06:00:00 AM','2017-05-24 02:00:00 PM','A','','',''  
**************************************************************************************************************/    
...
Select @strsql=''     
Select @strsql= 'insert into #Target(MachineID,machineinterface,Component,CompDescription,Compinterface,Operation,Opninterface,    
...
T.Pdate,T.Shift,T.shiftstart,T.Shiftend,0,autodata.id,componentoperationpricing.Cycletime FROM #T_autodata autodata    
...
END  
 ...
 SELECT distinct t.machineinterface,t.Compinterface,t.Opninterface,t.oprinterface,t.Pdate,t.shift,  
     STUFF(ISNULL((SELECT ', ' + x.downcode + ' [' + cast(dbo.f_FormatTime(sum(x.downtime),'hh:mm') as nvarchar(max))  + ']'  
     FROM #Downcode x  
...
Operator as OperatorName,Round(SUM(Target),0) as Target,SUM(Components) as Actual,SUM(stdtime) as StdCycleTime,Round(SUM(avgCycletime),2) as ActualCycleTime,  
Downtimecode,DowntimeDescription,dbo.f_FormatTime(SUM(Downtime),'hh:mm') as TotalDown,Round(AVG(OverAllEfficiency),0) as OE  
from #FinalTarget    
where (Utilisedtime>0 or Downtime>0 or Components>0)   
Group by Machineid,fromtm,shift,Operator,Component,CompDescription,Operation,Downtimecode,DowntimeDescription  
