...
select @curtime= getdate()
 ...
 Begin    
   ...
   from (     
   select A.Alarmno as AlarmNo,A.AlarmMSG as AlarmMessage,Count(A.AlarmNo) as NOofOccurences,Min(A.AlarmTime) as MinAlarmTime,Max(A.AlarmTime) as MaxAlarmtime,F.filePath,
