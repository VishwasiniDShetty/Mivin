/****** Object:  Procedure [dbo].[s_GetALFALAVAL_ShiftwiseMCOOLevelProdDownReport]    Committed by VersionSQL https://www.versionsql.com ******/
...
DR0375 - SwathiKS - 27/Jul/2017 :: To handle Double "TotalAvailableHours" when same M-C-O occurs within Same Shift.(e.g. M1-C1-O1 THEN M1-C2-O2 AGAIN M1-C1-O1)
ER0465 - Gopinath - 15/may/2018 :: To replace Cursor with PartitionBy for Performance Optimization.
--[dbo].[s_GetALFALAVAL_ShiftwiseMCOOLevelProdDownReport]   '2019-03-01 06:55:00 AM','2019-03-02 06:55:00 AM','','','','Format-2'
**************************************************************************************************************/  
CREATE PROCEDURE [dbo].[s_GetALFALAVAL_ShiftwiseMCOOLevelProdDownReport]  
 @StartDate datetime,  
 @EndDate datetime,  
...
--Select top 15 downid from downcodeinformation where --catagory not in('Management Loss')and
--SortOrder<=15 and SortOrder IS NOT NULL order by sortorder
Select top 17 downid from downcodeinformation where --catagory not in('Management Loss')and
(SortOrder>0 and SortOrder<=17) and SortOrder IS NOT NULL order by sortorder
...
Operator,Oprinterface,msttime,ndtime,FromTm,Totm,batchid,autodataid,stdtime,Shift)'  
...
T.shiftstart,T.Shiftend,0,autodata.id,componentoperationpricing.machiningtime,T.shift FROM #T_autodataforDown  autodata  
...
--insert into #FinalTarget (MachineID,Component,operation,Operator,machineinterface,Compinterface,Opninterface,Oprinterface,batchid,BatchStart,BatchEnd,FromTm,ToTm,Utilisedtime,Downtime,Components,ManagementLoss,MLDown,stdtime,shift
--,TotalAvailabletime)   
--Select MachineID,Component,operation,Operator,machineinterface,Compinterface,Opninterface,Oprinterface,batchid,min(msttime),max(ndtime),FromTm,ToTm,0,0,0 ,0,0,stdtime,shift,0 from #Target   
--group by MachineID,Component,operation,Operator,machineinterface,Compinterface,Opninterface,Oprinterface,batchid,FromTm,ToTm,stdtime,shift order by batchid   
  
  --g:
  insert into #FinalTarget (MachineID,Component,operation,Operator,machineinterface,Compinterface,Opninterface,Oprinterface,BatchStart,BatchEnd,FromTm,ToTm,Utilisedtime,Downtime,Components,ManagementLoss,MLDown,stdtime,shift,TotalAvailabletime,batchid)
select MachineID,Component,operation,Operator,machineinterface,Compinterface,Opninterface,Oprinterface,min(msttime),max(ndtime),FromTm,ToTm,0 Utilisedtime,0 Downtime,0 Components,0 ManagementLoss,0 MLDown,stdtime,shift,0 TotalAvailabletime,batchid
from
(
...
END
 ...
 /****** Swathi Commented from here Now TotalAvailabletime is ShiftHours and ActualAvailableTime is ShiftHours-PDT ***/
...
-- select Convert(nvarchar(10),FromTm,120) as PDate,Machineid,Operator,shift,Component,Operation,
--dbo.f_formattime(sum(TotalAvailabletime),'hh') as TotalAvailabletime,dbo.f_formattime(sum(CN),'hh') as Prodhrs 
...
--dbo.f_formattime(sum(N),'hh') as N,dbo.f_formattime(sum(O),'hh') as O,dbo.f_formattime(sum(others),'hh') as others,
--dbo.f_formattime(sum(Downtime),'hh') as TotalDowntime,dbo.f_formattime(sum(Utilisedtime),'hh') as ActualWorkinghrs,
...
--Group by Machineid,fromtm,shift,Operator,Component,Operation,stdtime
--Order by Machineid,FromTm

--Swathi Commented from here
--UPDATE #FinalTarget
--SET ActualAvailableTime= (CASE WHEN SHIFT = 'FIRST' THEN (7.20*3600) WHEN  SHIFT = 'SECOND' THEN (7.15*3600) WHEN SHIFT = 'THIRD' THEN (6.10*3600) END)
--Swathi Commented till here
...
Begin
	 ...
	 select Convert(nvarchar(10),FromTm,120) as PDate,Machineid,Operator,shift,Component,Operation,
	dbo.f_formattime(TotalAvailabletime,'hh') as TotalAvailabletime,dbo.f_formattime(sum(CN),'hh') as Prodhrs, 
	dbo.f_formattime(sum(A),'hh') as A,dbo.f_formattime(sum(B),'hh') as B,dbo.f_formattime(sum(C),'hh') as C,dbo.f_formattime(sum(D),'hh') as D,
	dbo.f_formattime(sum(E),'hh') as E,dbo.f_formattime(sum(F),'hh') as F,dbo.f_formattime(sum(G),'hh') as G,dbo.f_formattime(sum(H),'hh') as H,dbo.f_formattime(sum(I),'hh') as I,
	dbo.f_formattime(sum(J),'hh') as J,dbo.f_formattime(sum(K),'hh') as K,dbo.f_formattime(sum(L),'hh') as L,dbo.f_formattime(sum(M),'hh') as M,
	dbo.f_formattime(sum(N),'hh') as N,dbo.f_formattime(sum(O),'hh') as O,
	dbo.f_formattime(sum(P),'hh') as P,dbo.f_formattime(sum(Q),'hh') as Q, --SV
	dbo.f_formattime(sum(others),'hh') as others,
	dbo.f_formattime(sum(Downtime),'hh') as TotalDowntime,dbo.f_formattime(sum(Utilisedtime),'hh') as ActualWorkinghrs,
	Round(Sum(AvailabilityEfficiency),2) as AE,Round(Sum(ProductionEfficiency),2) as PE,Round(Sum(OverAllEfficiency),2) as OE,
	dbo.f_formattime(ActualAvailableTime,'hh') as ActualAvailabletime,ROUND(DeviationInActAvlTime,2) as DeviationInActAvlTime,
	ROUND(SUM(TargetCount),2) as TargetCount,SUM(components) as ActualCount,dbo.f_formattime(sum(Reworkhrs),'hh') as Reworkhrs
	into #FinalOutout from #FinalTarget  
	where (Utilisedtime>0 or Downtime>0 or Components>0) 
	Group by Machineid,fromtm,shift,Operator,Component,Operation,stdtime,TotalAvailabletime,ActualAvailableTime,DeviationInActAvlTime,Reworkhrs
	Order by Machineid,FromTm

	update #FinalOutout set TotalDowntime=T.TotalDowntime,Prodhrs=T.Prodhrs,Reworkhrs=T.Reworkhrs from
	(select PDate,Machineid,shift,SUM(cast(TotalDowntime as float)) as TotalDowntime,SUM(cast(Prodhrs as float)) as Prodhrs,SUM(cast(Reworkhrs as float)) as Reworkhrs from #FinalOutout
	group by PDate,Machineid,shift)T
	inner join #FinalOutout on #FinalOutout.pdate=T.PDate and #FinalOutout.Machineid=T.Machineid and #FinalOutout.shift=T.shift
	
	Select PDate,Machineid,Operator,shift,Component,Operation,TotalAvailabletime,Prodhrs,A,B,C,D,E,F,G,H,I,J,K,L,M,N,O,P,Q,others,TotalDowntime,ActualWorkinghrs,
	AE,PE,OE,ActualAvailabletime,DeviationInActAvlTime,TargetCount,ActualCount,Reworkhrs from #FinalOutout Order by Machineid,PDate
