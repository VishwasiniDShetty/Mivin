/****** Object:  Procedure [dbo].[s_GetCockpitData]    Committed by VersionSQL https://www.versionsql.com ******/
...
ER0459 - SwathiKS - 01/mar/2018 :: To show Proper JobCode For BaluAuto.
...
s_GetCockpitData '2017-11-01 06:00:00 AM','2017-11-03 02:00:00 PM','',''  
****************************************************************************************************/    
CREATE PROCEDURE [dbo].[s_GetCockpitData]    
...
-- exec (@strsql)   
	...
	SELECT @strsql= @strsql + 'insert into #Runningpart_Part(Machineid,Componentid,StTime)  
	  ...
	  (
		Select mc,max(sttime) as sttime from autodata 
		where sttime>='''+convert(nvarchar(20),@starttime)+''' and ndtime<='''+convert(nvarchar(20),@endtime)+''' 
		group by mc
	  ) A  
	  inner join autodata T on T.mc=A.mc and T.sttime=A.sttime
