...
BEGIN
	...
	(Select machineid,Sum(Target) as Tcount from #FinalTarget
	 Group by machineid)T1 inner join #CockpitData on #CockpitData.machineid=T1.machineid 
		...
		OEGreen,
		OERed,
		dbo.f_FormatTime(Case when ISNULL(HelpCode1TS,0)>0 and HelpCode1TS<=60 then 60 else HelpCode1TS END,'hh:mm') as HelpCode1TS,
		dbo.f_FormatTime(Case when ISNULL(HelpCode2TS,0)>0 and HelpCode2TS<=60 then 60 else HelpCode2TS END,'hh:mm') as HelpCode2TS,
		dbo.f_FormatTime(Case when ISNULL(HelpCode3TS,0)>0 and HelpCode3TS<=60 then 60 else HelpCode3TS END,'hh:mm') as HelpCode3TS,
		dbo.f_FormatTime(Case when ISNULL(HelpCode4TS,0)>0 and HelpCode4TS<=60 then 60 else HelpCode4TS END,'hh:mm') as HelpCode4TS
		FROM #CockpitData
...
Begin
	insert into #FinalTarget (MachineID,Component,operation,machineinterface,Compinterface,Opninterface,Runtime,FromTm,ToTm,Targetpercent,Suboperation,StdCycletime) 
	Select MachineID,Component,operation,machineinterface,Compinterface,Opninterface,datediff(s,FromTm,ToTm),FromTm,ToTm,Targetpercent,Suboperation,StdCycletime from #Target WITH (NOLOCK)
	group by MachineID,Component,operation,machineinterface,Compinterface,Opninterface,FromTm,ToTm,Targetpercent,Suboperation,StdCycletime

	 Update #FinalTarget set Target = Isnull(Target,0) + isnull(T1.targetcount,0) from
	(Select machineinterface,Compinterface,Opninterface,FromTm,sum(((Runtime*suboperation)/stdcycletime)*isnull(targetpercent,100) /100) as targetcount
	 from #FinalTarget group by machineinterface,Compinterface,Opninterface,FromTm)T1 inner join #FinalTarget on  T1.machineinterface=#FinalTarget.machineinterface and T1.Compinterface=#FinalTarget.Compinterface
	and T1.Opninterface=#FinalTarget.Opninterface and T1.FromTm=#FinalTarget.FromTm
End
...
/*******************************      Utilised Calculation Starts ***************************************************/
		...
		from
		(select mc,
		sum(DateDiff(second, @StartTime, @EndTime)) cycle from  autodata WITH (NOLOCK) -- ----ER0377 Added LOCK
		where (autodata.msttime<@StartTime)
		and (autodata.ndtime>@EndTime)
		and (autodata.datatype=1)
		group by autodata.mc
		...
		--			WHEN autodata.msttime >= T.StartTime  AND autodata.ndtime <=T.EndTime  THEN (autodata.cycletime+autodata.loadunload) --DR0325 Commented
			...
			Select T1.mc,SUM(
				...
				(Select A.mc,(select machineid from machineinformation where interfaceid = A.mc)as machine,
				 A.sttime, ndtime, A.datatype from autodata A WITH (NOLOCK) -- ----ER0377 Added LOCK
				Where A.DataType=2
				and exists 
				(
				Select B.Sttime,B.NdTime From autodata B WITH (NOLOCK) -- ----ER0377 Added LOCK
		...
		BEGIN
			...
			(
				select mc,SUM((O.cycletime * ISNULL(A.PartsCount,1))/ISNULL(O.SubOperations,1))  C1N1
				From autodata A WITH (NOLOCK) -- ----ER0377 Added LOCK
				Inner join machineinformation M on M.interfaceid=A.mc
				Inner join componentinformation C ON A.Comp=C.interfaceid
