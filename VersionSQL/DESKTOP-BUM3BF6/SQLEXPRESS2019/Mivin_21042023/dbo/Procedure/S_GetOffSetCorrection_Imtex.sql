...
[S_GetOffSetCorrection_Imtex] '2022-02-28 00:00:00.000','2022-03-10 00:00:00.000','','','R983032308','Maini'
...
BEGIN
	...
	BEGIN
		...
		A.Value as MeasureDimension,A.CorrectionValue,A.MeasureDimension As MeanValue,E.Employeeid from SPCAutodata A
		inner join Machineinformation on A.mc=Machineinformation.interfaceid
...
SET @ST= @StartTime
 ...
 INSERT INTO #spcautodata(mc,comp,opn,opr,TimeStamp,Dimension,WearOffsetNumber,value,correctionvalue,Measuredimension,OvalityMax,OvalityMin)
 SELECT mc,comp,opn,opr,Timestamp,Dimension,WearOffSetNumber,Value,CorrectionValue,MeasureDimension,OvalityMax,OvalityMin  FROM dbo.SPCAutodata s
...
if  ((select ValueInText FROM ShopDefaults WHERE Parameter = 'SPCCompOpnSet') = 'SPCMaster')
	BEGIN
		...
		SP.LSL,SP.USL,(Case When A.Value >= SP.LSL and A.value <= SP.USL then 0
						...
						END) as ValueOutOfRange,A.OvalityMax AS MaxVal,a.OvalityMin as MinVal,round((cast(A.OvalityMax as float)-cast(a.OvalityMin as float)),2) as Ovality
		from #spcautodata A
		inner join Machineinformation on A.mc=Machineinformation.interfaceid
		Left Outer join Employeeinformation E on A.opr=E.interfaceid --Added For Autotech
		inner join SPC_Characteristic SP on Machineinformation.machineid=SP.machineid and A.opn=SP.operationno and A.Dimension=SP.CharacteristicID
	...
	ELSE
	BEGIN
		SELECT ROW_NUMBER() OVER (ORDER BY A.TimeStamp desc) AS CycleNo, machineinformation.machineid,ci.componentid,S.ShiftDate,S.ShiftName,A.TimeStamp as MeasuredTime, A.Dimension,A.WearoffSetNumber,
		A.Value as MeasureDimension,A.CorrectionValue,A.MeasureDimension As MeanValue,E.Employeeid,
		SP.LSL,SP.USL,(Case When A.Value >= SP.LSL and A.value <= SP.USL then 0
							when A.Value < SP.LSL and A.value > SP.USL Then 1
							else 1
