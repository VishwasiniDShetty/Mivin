...
SET @strSql = 'INSERT INTO #CockpitData (
			...
			SET @strSql = @strSql + ' SELECT S.Pdate,S.FromTime,S.ToTime,S.ShiftName,MachineInformation.MachineID, MachineInformation.interfaceid ,Machineinformation.Description,
						  0,0,0,0,0,0,0,0,0 FROM MachineInformation
...
update #cockpitdata set AvgProductionEfficiency=isnull(t2.APE,0),AvgAvailabilityEfficiency = isnull(t2.aae,0),AvgOverallEfficiency=isnull(t2.OEE,0)  from 
 (select MachineId,avg(ProductionEfficiency)*100 as APE,avg(AvailabilityEfficiency)*100 as AAE,(avg(ProductionEfficiency * AvailabilityEfficiency)*100) As OEE  from #CockpitData 
...
if @TimeAxis='Shift' AND @Type='Console' and @daywise='1'
	BEGIN
		...
		--'OE' = CASE
			...
			--END,
		'OE' = isnull(CASE
				WHEN isnull(Components,0) <> 0 then ROUND((ProductionEfficiency * AvailabilityEfficiency*100) * ((Components - isnull(rejection,0))/Components),2)
			END,0),
		---mod 3
		Components,Rejection,
