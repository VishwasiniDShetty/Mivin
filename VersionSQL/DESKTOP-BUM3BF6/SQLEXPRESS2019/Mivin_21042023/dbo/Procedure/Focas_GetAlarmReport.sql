...
--[dbo].[Focas_GetAlarmReport]  '2017-06-17 06:00:00','2017-06-18 06:00:00','MACHINE-11','','details',''  
...
BEGIN  
 ...
 Begin      
  ...
  left outer join Focas_AlarmCategory c on  a.AlarmGroupNo = c.AlarmNo      
  where (AlarmTime between FromTime and  ToTime) and (machineId=@machineid or ISNULL(@machineid,'')='')    
...
BEGIN  
 ...
 Begin      
   ...
   where (AlarmTime between @StartTime and @EndTime)  and   
   ( a.AlarmNo < 1150  OR a.AlarmNo > 1172 )   
   and (machineId=@machineid or ISNULL(@machineid,'')='') Group by A.MachineID,A.Alarmno,A.AlarmMSG,F.FilePath,A.Ackstatus)T     
   Order by T.MaxAlarmtime desc  
   End          
   ELSE      
   Begin       
   ...
   and (machineId=@machineid or ISNULL(@machineid,'')='') and (AlarmTime between @StartTime and @EndTime)     
...
IF isnull(@AlarmGroup,'ALL')= 'ALL' or @AlarmGroup=''      
 Begin      
   ...
   left outer join (SELECT DISTINCT Alarmno,flag from Focas_AlarmMaster) F on A.Alarmno=F.AlarmNo      
   left outer join Focas_AlarmCategory c on  a.AlarmGroupNo = c.AlarmNo      
   where (AlarmTime between @StartTime and  @EndTime) and (machineId=@machineid or ISNULL(@machineid,'')='')     
   order by AlarmTime desc      
 End      
...
ELSE      
 Begin      
  ...
  left outer join (SELECT DISTINCT Alarmno,flag from Focas_AlarmMaster) F on A.Alarmno=F.AlarmNo      
  left outer join Focas_AlarmCategory c on  a.AlarmGroupNo = c.AlarmNo      
  where AlarmGroupNo =  @AlarmGroup and (machineId=@machineid or ISNULL(@machineid,'')='') and (AlarmTime between @StartTime and @EndTime)      
  order by AlarmTime desc      
 End      
End      
...
IF isnull(@AlarmGroup,'ALL')= 'ALL' or @AlarmGroup=''      
 Begin      
   ...
   inner join (SELECT DISTINCT Alarmno,flag from Focas_AlarmMaster where flag='1') F on A.Alarmno=F.AlarmNo      
   left outer join Focas_AlarmCategory c on  a.AlarmGroupNo = c.AlarmNo      
   where (AlarmTime between @StartTime and  @EndTime) and (machineId=@machineid or ISNULL(@machineid,'')='') and F.flag='1'      
   order by AlarmTime desc      
 End      
...
IF isnull(@AlarmGroup,'ALL')= 'ALL' or @AlarmGroup=''      
 Begin      
   ...
   inner join (SELECT DISTINCT Alarmno,flag from Focas_AlarmMaster where flag='2') F on A.Alarmno=F.AlarmNo      
   left outer join Focas_AlarmCategory c on  a.AlarmGroupNo = c.AlarmNo      
   where (AlarmTime between @StartTime and  @EndTime) and (machineId=@machineid or ISNULL(@machineid,'')='') and F.flag='2'      
   order by AlarmTime desc      
 End      
