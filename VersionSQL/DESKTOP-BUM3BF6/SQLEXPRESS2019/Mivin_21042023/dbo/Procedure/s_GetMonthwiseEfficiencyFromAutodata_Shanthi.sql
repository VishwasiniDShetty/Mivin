...
Declare @StrGroupID AS NVarchar(900)
...
SET @strSql = 'INSERT INTO #CockpitData (    
   ...
   SET @strSql = @strSql + @strPlantID + @strMachine + @StrGroupID  
   EXEC(@strSql)    
--vasavi     
...
BEGIN    
 ...
 inner join #CockpitData C on C.machineid=P.machine    
  where P.starttime>=C.Strttm and P.endtime<=C.ndtim group by P.Machine,C.Strttm)T1    
...
SELECT T1.MachineID,T1.GroupID,T1.Components,T1.RejCount,T1.ProductionEfficiency,T1.AvailabilityEfficiency,T1.QualityEfficiency,T1.OverAllEfficiency,    
T1.AvgOverallEfficiency,[dbo].[f_FormatTime] (T1.Totaltime,'hh:mm:ss') as Totaltime,[dbo].[f_FormatTime] (T1.downtime,'hh:mm:ss') as downtime,[dbo].[f_FormatTime] (T1.UtilisedTime,'hh:mm:ss') as UtilisedTime,    
T2.MachineID as MachineID1,T2.Components as Components1,T2.RejCount as RejCount1,T2.ProductionEfficiency as PE,T2.AvailabilityEfficiency as AE,T2.QualityEfficiency as QE,T2.OverAllEfficiency as OE,    
T2.AvgOverallEfficiency as AvgOE,[dbo].[f_FormatTime] (T2.Totaltime,'hh:mm:ss') as Totaltime1,[dbo].[f_FormatTime] (T2.downtime,'hh:mm:ss') as downtime1,[dbo].[f_FormatTime] (T2.UtilisedTime,'hh:mm:ss') as UtilisedTime1    
FROM (Select * from #CockpitData where datepart(month,Strttm)=datepart(month,@starttime)) T1 join     
(Select * from #CockpitData where datepart(month,Strttm)=datepart(month,dateadd(month,-1,@starttime))) T2 on T1.machineid=T2.machineid     
Order by T1.Machineid,T1.Strttm    
----------- DR0379 Till Here  
