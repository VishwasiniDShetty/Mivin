...
ER0370 - SwathiKS - 06/Dec/2013 :: AlarmData was displaying irrespective of Machine Selection.
*************************************************************************/
--s_GetMachineAlarm '2013-12-06 06:00:00' , '2013-12-06 10:30:32','ACE VTL-01'
...
--SELECT @StrSql=@StrSql + ' WHERE A.AlarmTime>='''+ CONVERT(NVarChar,@StartTime)+ ''' AND  A.AlarmTime<=''' + CONVERT(NVarChar,@EndTime) + ''' AND A.RecordType=6 or A.RecordType=85' --ER0370
SELECT @StrSql=@StrSql + ' WHERE A.AlarmTime>='''+ CONVERT(NVarChar,@StartTime)+ ''' AND  A.AlarmTime<=''' + CONVERT(NVarChar,@EndTime) + ''' AND (A.RecordType=6 or A.RecordType=85)' --ER0370
...
FETCH NEXT FROM TmpCursor Into @CurMchID,@CurAlNumber
END
close TmpCursor --ER0370
deallocate TmpCursor --ER0370
