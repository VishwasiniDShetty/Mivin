...
Declare @StrGroup as nvarchar(255)     
Declare @StrGroupid as nvarchar(255)  
DECLARE @joined NVARCHAR(4000)
---mod 1
-- Temporary Table
...
--	select @strsql =  @strsql + ' and ( Machineinformation.machineid = ''' + @machineid + ''')'
	...
	select @strsql =  @strsql + ' and ( Machineinformation.machineid in (' + @joined + '))'
...
If isnull(@Groupid,'') <> ''  
Begin  
 Select @StrGroupid = ' And ( SD.GroupID = N''' + @GroupID + ''')'  
End 
...
select @strmachine=' AND (SD.machineid in (' + @joined + '))' --ER0453 
---mod 1
end
