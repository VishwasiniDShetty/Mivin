...
SubOperations, StdSetupTime, MachiningTimeThreshold, TargetPercent,LoadUnloadTimeThreshold,SCIThreshold,DCLThreshold,ID,FinishedOperation,MinLoadUnloadThreshold
into #tmpcomponent
FROM ItemMaster
...
SELECT distinct  ItemNo, Iteminterfaceid, Itemdescription,customerid ,Operationno, Opndescription, [CNC M/C], Price, Drawingno, Opninterfaceid, LoadUnloadTime, CycleTime, 
SubOperations, StdSetupTime, MachiningTimeThreshold, TargetPercent,LoadUnloadTimeThreshold,SCIThreshold,DCLThreshold,ID,FinishedOperation,MinLoadUnloadThreshold from #tmpcomponent
...
Insert into Componentoperationpricing(componentid, operationno, description, machineid, price, cycletime, drawingno, InterfaceID, loadunload, machiningtime, SubOperations, StdSetupTime, 
                      ...
                      McTimeMonitorLThreshold, McTimeMonitorUThreshold, StdDieCloseTime, StdPouringTime, StdSolidificationTime, StdDieOpenTime,FinishedOperation,MinLoadUnloadThreshold
)
...
0 as UpperEnergyThreshold,T.SCIThreshold,T.DCLThreshold,0 as McTimeMonitorLThreshold,0 as McTimeMonitorUThreshold,0 as StdDieCloseTime,0 as StdPouringTime,0 as StdSolidificationTime,0 as StdDieOpenTime,FinishedOperation,MinLoadUnloadThreshold
FROM 
	(select distinct itemno,operationno,Opndescription,Opninterfaceid,[CNC M/C],price,Cycletime,drawingno,
	SubOperations,StdSetupTime,MachiningTimeThreshold,TargetPercent,Loadunloadtime,LoadUnloadTimeThreshold,SCIThreshold,DCLThreshold,FinishedOperation,MinLoadUnloadThreshold from itemmaster
	where itemno+[CNC M/C]+ convert(nvarchar(20),operationno)  not in 
	(select componentoperationpricing.Componentid+componentoperationpricing.machineid+ convert(nvarchar(20),componentoperationpricing.operationno)
...
begin
	...
	Machiningtime=T.cycletime,SCIThreshold=T.SCIThreshold,DCLThreshold=T.DCLThreshold,FinishedOperation=t.finishedoperation,MinLoadUnloadThreshold=t.MinLoadUnloadThreshold from 
	(select distinct itemno,operationno,opndescription,opninterfaceid,[CNC M/C],Cycletime,Loadunloadtime,LoadUnloadTimeThreshold,SCIThreshold, DCLThreshold,FinishedOperation,MinLoadUnloadThreshold from itemmaster
	where itemno+[CNC M/C]+ convert(nvarchar(20),operationno)  in 
	(select componentoperationpricing.Componentid+componentoperationpricing.machineid+ convert(nvarchar(20),componentoperationpricing.operationno)
