/****** Object:  Procedure [dbo].[s_GetMachineProductionData_v01]    Committed by VersionSQL https://www.versionsql.com ******/

create  PROCEDURE s_GetMachineProductionData_v01
	@StartTime as DateTime, @EndTime as DateTime, @MachineID as nvarchar(50) = '',
...
select @strsql = 'INSERT INTO #ProductionData (MachineID, Production, Rejection, CN, TurnOver)'
select @strsql = @strsql + ' SELECT  workorderheader.MachineID, SUM(workorderproductiondetail.production) AS
Production, '
...
exec (@strsql)
/*
INSERT INTO #ProductionData (MachineID, Production, Rejection, CN, TurnOver)
...
--	WHERE 	
--		( workorderheader.machineid LIKE '%'+#ProductionData.MachineID+'%')
--		AND
--		( workorderheader.componentid LIKE '%'+@ComponentID+'%' )
--		AND
