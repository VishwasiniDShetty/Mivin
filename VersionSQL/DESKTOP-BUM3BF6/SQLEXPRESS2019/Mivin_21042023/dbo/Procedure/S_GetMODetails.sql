...
--[dbo].[S_GetMODetails] '2018-10-01 06:00:00 AM','2018-10-30 06:00:00 AM','','SP-15_A',''
...
----ER0465 Commented Cursor and added PartitionBy
...
--ER0465 g: (MachineID,machineinterface,Component,Compinterface,msttime,ndtime,batchid,runtime,autodataid,autodataWO)
  insert into #FinalRuntime (MachineID,Component,machineinterface,Compinterface,Runtime,msttime,ndtime,autodatawo,batchid) 
select MachineID,Component,machineinterface,Compinterface,datediff(s,min(msttime),max(ndtime)),min(msttime),max(ndtime),autodatawo, batchid
from
(
select MachineID,Component,machineinterface,Compinterface,msttime,ndtime,autodatawo,
RANK() OVER (
  PARTITION BY t.machineid
  order by t.msttime
) -
RANK() OVER (
  PARTITION BY  t.machineid, t.component, t.autodataWO
  order by t.msttime
) AS batchid
from #Runtime t 
) tt
group by MachineID,Component,batchid,machineinterface,Compinterface,autodatawo
order by tt.batchid
-- ER0465 g:/
