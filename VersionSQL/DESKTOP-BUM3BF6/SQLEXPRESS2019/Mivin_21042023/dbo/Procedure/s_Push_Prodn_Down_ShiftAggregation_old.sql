/****** Object:  Procedure [dbo].[s_Push_Prodn_Down_ShiftAggregation_old]    Committed by VersionSQL https://www.versionsql.com ******/
...
--s_Push_Prodn_Down_ShiftAggregation  '2021-02-17'   , '' ,'''CNC-44''' ,'','PUSH','','2021-02-17'
...
,FormatedWONumber,GroupID)' --ER0349 added New Column --GNA
--DR0272 Till Here
---mod 7
...
---mod 7
SELECT @StrSql= @StrSql+',A.WorkOrderNumber,PlantMachineGroups.GroupID ' --ER0349 Added --GNA
SELECT @StrSql= @StrSql
...
---mod 7
SELECT @StrSql = @StrSql + ',A.WorkOrderNumber,machineinformation.MachinewiseOwner ,machineinformation.CriticalMachineEnabled,PlantMachineGroups.GroupID '--geeta added --GNA
---mod 7
print @StrSql
Exec (@StrSql)
...
Price,SubOperation,LastAggrStart,Minterface,Cinterface ,Opninterface ,OprInterface,WorkOrderNumber,
MachinewiseOwner ,CriticalMachineEnabled --geeta added
,FormatedWONumber,GroupID)' --ER0349 Added New column
--DR0272 Till Here
---mod 7
SELECT @StrSql= @StrSql
+ ' SELECT T.Mdate,T.Mshift,T.MShiftStart,T.MShiftEnd,PlantMachine.PlantID,T.Amachine,
...
---mod 7
SELECT @StrSql = @StrSql + ',PlantInformation.Plantcode,PlantMachineGroups.GroupID' --ER0349 Added  --Added Groupid For GNA
print @StrSql
Exec (@StrSql)
...
from
(select   T.pDate AS CDate,T.Shift AS CShift, T.Machineid AS CMachine,T.componentid AS CComponent,T.OperationNo AS COpnNo,T.OperatorID AS COpr,
CAST(CEILING(CAST(Sum(A.PartsCount)as float)/ ISNULL(T.SubOperation,1))as integer ) as Components,T.WorkOrderNumber as WorkOrderNumber
from #T_autodata A -----ER0202 
Inner join #TempShiftProductionDetails T on
T.Minterface=A.mc and T.Cinterface=A.comp and T.Opninterface=A.opn and T.OprInterface=A.opr and A.WorkOrderNumber = T.WorkOrderNumber
where (A.ndtime>T.PShiftStart and A.ndtime<=T.PShiftEnd and A.datatype=1 and A.msttime>=T.LastAggrStart)'
SELECT @StrSql=@StrSql +' group by T.pDate,T.Shift,T.Machineid,T.componentid,T.OperationNo ,T.OperatorID,T.WorkOrderNumber,T.SubOperation
) as t2 inner join #TempShiftProductionDetails on t2.CDate=#TempShiftProductionDetails.pDate
And t2.CShift=#TempShiftProductionDetails.Shift
And t2.CMachine = #TempShiftProductionDetails.MachineID
And t2.CComponent=#TempShiftProductionDetails.Componentid
...
---mod 7
#TempShiftProductionDetails.WorkOrderNumber as WorkOrderNumber,
---mod 7
...
from #T_autodata A ---ER0202 
Inner join #TempShiftProductionDetails T on
A.mc= T.Minterface and A.comp=T.Cinterface and T.Opninterface=A.opn and A.Opr=T.OprInterface and A.WorkOrderNumber = T.WorkOrderNumber
where ((A.msttime>=T.PShiftStart and A.ndtime<=T.PShiftEnd)
or (A.msttime<T.PShiftStart and A.ndtime>T.PShiftStart and A.ndtime<=T.PShiftEnd)
or (A.msttime>=T.PShiftStart and A.msttime<T.PShiftEnd and A.ndtime>T.PShiftEnd)
or (A.msttime<T.PShiftStart and A.ndtime>T.PShiftEnd) )
...
---mod 7
-- (Idate,IShift,IShiftStart,IShiftEnd,Imc,Icomp,Iopn,Iopr,Isttime,Indtime)
(Idate,IShift,IShiftStart,IShiftEnd,Imc,Icomp,Iopn,Iopr,Imsttime,Isttime,Indtime,WorkOrderNumber)
---mod 7
Select T.pDate AS PDate,T.Shift AS IShift,T.PShiftStart as shftStart,T.PShiftEnd as shiftend,
...
T1.Iopn AS COpnNo,T1.Iopr AS COpr,T1.WorkOrderNumber,
sum(CASE
When A1.sttime <= IShiftStart Then datediff(s, IShiftStart,A1.ndtime )
When A1.sttime > IShiftStart Then datediff(s , A1.sttime,A1.ndtime)
END) as Down  FROM 
	(
	Select I.Icomp,I.Iopn,I.iopr,I.Imc,I.Isttime,I.Indtime,I.IShiftEnd,I.IShiftStart,I.WorkOrderNumber,I.Idate,I.IShift From #ICD I
	...
	And I.Iopr=T.OprInterface and I.WorkOrderNumber = T.WorkOrderNumber	  
	Where (I.Imsttime < T.PShiftStart)And (I.Indtime > T.PShiftStart) AND (I.Indtime <= T.PShiftEnd)
	)  T1 
inner join #T_autodata A1 on A1.mc=T1.Imc and A1.comp=T1.Icomp and A1.opn=T1.Iopn and A1.opr=T1.Iopr and A1.WorkOrderNumber=T1.WorkOrderNumber -----ER0202 
where A1.datatype=2 and (A1.sttime>T1.Isttime and A1.ndtime<T1.Indtime and A1.ndtime>T1.IShiftStart)
Group by T1.Idate,T1.IShift,T1.Imc,T1.Icomp,T1.Iopn,T1.Iopr,T1.WorkOrderNumber
)AS T2
Inner join #TempShiftProductionDetails on t2.CDate=#TempShiftProductionDetails.pDate
And t2.CShift = #TempShiftProductionDetails.Shift
...
T1.Opninterface AS COpnNo,T1.OprInterface AS COpr,T1.WorkOrderNumber as WorkOrderNumber,
SUM(CASE
...
and A.WorkOrderNumber=T1.workordernumber --DR0322 Added Line
...
OR ( A.msttime < T.StartTime  AND A.ndtime > T.EndTime) ) 
and A.msttime>=T1.LastAggrStart --DR0322 Added Line
group by T.Sdate,T.ShiftName,T1.Minterface,T1.Cinterface,T1.Opninterface,T1.OprInterface,T1.WorkOrderNumber
,T1.PShiftStart,T1.PShiftEnd --DR0322 Added Line
)AS T2 
Inner join #TempShiftProductionDetails on t2.CDate=#TempShiftProductionDetails.pDate
And t2.CShift=#TempShiftProductionDetails.Shift
...
T1.Iopn AS COpnNo,T1.Iopr AS COpr,T1.WorkOrderNumber as WorkOrderNumber,
SUM(
CASE 
When autodata.sttime >= T.StartTime  AND autodata.ndtime <=T.EndTime  Then datediff(s , autodata.sttime,autodata.ndtime) ---type 1
When autodata.sttime < T.StartTime  and  autodata.ndtime <= T.EndTime AND autodata.ndtime > T.StartTime Then datediff(s, T.StartTime,autodata.ndtime ) ---type 2
...
ON autodata.mc=T1.Imc And autodata.Comp=T1.Icomp And autodata.Opn=T1.IOpn And autodata.Opr=T1.IOpr and autodata.WorkOrderNumber=T1.WorkOrderNumber
...
And ((autodata.sttime >= T.StartTime and autodata.ndtime <=T.EndTime)
    ...
    OR (autodata.sttime >= T.StartTime   AND autodata.sttime <T.EndTime AND autodata.ndtime > T.EndTime)
    OR (autodata.sttime < T.StartTime  AND autodata.ndtime > T.EndTime))
Group by T1.Idate,T1.IShift,T1.Imc,T1.Icomp,T1.Iopn,T1.Iopr,T1.WorkOrderNumber,T1.IShiftStart,T1.IShiftEnd
)AS T2
Inner join #TempShiftProductionDetails on t2.CDate=#TempShiftProductionDetails.pDate
AND t2.shiftstart=#TempShiftProductionDetails.PShiftStart and t2.shiftend=#TempShiftProductionDetails.PShiftend --DR0322 added
And t2.CShift=#TempShiftProductionDetails.Shift
...
---mod 7
and MachinewiseOwner= @MachinewiseOwner and CriticalMachineEnabled=@CriticalMachineEnabled)--geeta added
   begin
...
else T.MinLoadUnloadTime end as MinLoad from #TempShiftProductionDetails T
...
---mod 7
--Geeta Added from here
and T.MachinewiseOwner= @MachinewiseOwner
and T.CriticalMachineEnabled=@CriticalMachineEnabled)
-- geeta added till here
as T1
...
---mod 7
,MachinewiseOwner ,CriticalMachineEnabled,GroupID)--geeta added --GNA
...
---MOD 4(3)
  ...
  ,@WorkOrderNumber
  ---mod 7
 ,@MachinewiseOwner ,@CriticalMachineEnabled,@Grpid --geeta added --GNA
 END
...
Declare @Param1 As VarChar(4000) --DR0302 Added
Declare @Param2 As NVarChar(100)
--Declare @Param3 As NVarChar(2500) --DR0302 Commented
Declare @Param3 As VarChar(3500) --DR0302 Added
...
CASE WHEN Ndtime >T.MShiftEnd THEN T.MShiftEnd ELSE NdTime End)-ISNULL(Td.PLD_LoadUnload,0))>0'
END
---mod 11
...
--mod 7
SELECT @StrSql=@StrSql + ',CriticalMachineEnabled,Groupid)'--Geeta added --GNA
...
SELECT @StrSql=@StrSql + convert(nvarchar(1000),@Param2)
--DR0302 till here
---mod 11
...
---mod 1
---mod 7
SELECT @StrSql = @StrSql + ',Isnull(A.WorkOrderNumber,0)'
---mod 7
...
SELECT @StrSql=@StrSql + convert(nvarchar(4000),@Param1)
--DR0302 till here
---mod 11
--DR0302 from here
Select @strtemp=''
...
SELECT @StrSql = @StrSql+convert(nvarchar(1000),@Param3)
--DR0302 till here
---mod 11
...
SELECT @StrSql=@StrSql + ',PE_Flag,WorkOrderNumber'
--mod 7
SELECT @StrSql=@StrSql + ',CriticalMachineEnabled,Groupid)'--Geeta added
SELECT @StrSql=@StrSql + 'select T.Mdate,T.MShift,PlantMachine.PlantID,AMachine,
isnull(c.componentid,''Invalid Component'')  ,isnull(o.operationno,0) ,
