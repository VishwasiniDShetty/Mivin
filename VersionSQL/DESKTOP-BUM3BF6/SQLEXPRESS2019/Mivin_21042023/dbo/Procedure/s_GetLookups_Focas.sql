...
SET @strsql = 'Select distinct pm.Plantid from plantmachine pm inner join machineinformation mi
	on   pm.machineid=mi.machineid and  FocasEnabled=1 order by pm.plantid '
...
BEGIN
SET @strsql = ''
SET @strsql = 'select Machineid from machineinformation where  FocasEnabled=1 order by machineid '
SET @strsql = @strsql + @order
EXEC (@strsql)
END
...
INNER JOIN machineinformation mi
  ON pm.Machineid = mi.Machineid
   -- AND (TpmTrakEnabled = 1 OR FocasEnabled = 1)
   AND FocasEnabled = 1
...
BEGIN
SET @strsql = ''
SET @strsql = 'select distinct C.Componentid from componentoperationpricing C inner join Machineinformation M on M.machineid=C.machineid  
inner join PlantMachine P on P.MachineID=M.Machineid where  M.DNCTransferEnabled=1'
...
BEGIN
SET @strsql = ''
SET @strsql = 'select distinct C.Componentid from componentoperationpricing C inner join Machineinformation M 
on M.machineid=C.machineid inner join PlantMachine P on P.MachineID=M.Machineid  where  M.DNCTransferEnabled=1 and M.Machineid='
SET @strsql = @strsql + '''' + @filter + ''''
SET @strsql = @strsql + ' order by C.Componentid '
SET @strsql = @strsql + @order
EXEC (@strsql)
...
BEGIN
SET @strsql = ''
SET @strsql = 'select  distinct Operationno from componentoperationpricing COP inner join  Componentinformation CI on
COP.componentid=CI.componentid  inner join machineinformation m on COP.machineid=M.machineid '
