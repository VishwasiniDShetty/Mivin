/****** Object:  Procedure [dbo].[s_GetFocasLiveDetailsForMultipleMac]    Committed by VersionSQL https://www.versionsql.com ******/
 ...
 --[dbo].[s_GetFocasLiveDetailsForMultipleMac]'2018-12-04 10:00:00','first','','','',''            
...
-- exec [dbo].[s_GetFocasLiveDetailsForMultipleMac] @date=N'2017-06-17',@ShiftName=N'Day',@plantId=N'DAS CNC',@MachineId=N'',@param=N''    
...
FROM (Select ShiftDate,Shiftname,MachineStatus,RunningProgram,TotalTime,Total,[Powerontime],[Operating time], [Cutting time],Machineid,OEE,TotalPartsCount, AvgOEE,replace(Programs,',','.') Programs from #Live) t              
...
FROM (Select  [From Time],[To Time],MachineStatus,RunningProgram,TotalTime,[Powerontime],[Operating time], Total,[Cutting time],Machineid,OEE,AvgOEE,TotalPartsCount,replace(Programs,',','.') Programs from #Live1) t              
