...
@MacroLocation int='',
@CustomerLSL nvarchar(50)='',
@CustomerUSL nvarchar(50)='',
@Param nvarchar(50)
...
BEGIN
	...
	Select @NewCharacteristicID = Isnull(Max(CharacteristicID),0) from SPC_Characteristic where ComponentID=@ComponentID and OperationNo=@OperationNo
	select @NewCharacteristicID = @NewCharacteristicID + 1

	if not exists(select * from SPC_Characteristic where ComponentID=@ComponentID and OperationNo=@OperationNo and CharacteristicID=@CharacteristicID)
	BEGIN
	
		Insert into SPC_Characteristic(MachineID, ComponentID, OperationNo, CharacteristicCode, CharacteristicID, SpecificationMean, LSL, USL,
		Specification,CuLSL,CuUSL)
		Select  @MachineID, @ComponentID, @OperationNo, @CharacteristicCode, @NewCharacteristicID, @SpecificationMean, @LSL, @USL,
		@Specification,@CustomerLSL,@CustomerUSL
	END
	else
	BEGIN
	 update SPC_Characteristic set 	CharacteristicCode=@CharacteristicCode,SpecificationMean=@SpecificationMean,LSL=@LSL, USL=@USL,
	 CuLSL=@CustomerLSL,CuUSL=@CustomerUSL,
	Specification=@Specification where  ComponentID=@ComponentID and OperationNo=@OperationNo and CharacteristicID=@CharacteristicID
	END
...
If @param='DeleteCO'
Begin
	Delete From SPC_Characteristic where  ComponentID=@ComponentID and OperationNo=@OperationNo and CharacteristicID=@CharacteristicID
END
